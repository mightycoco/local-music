...
class M3uPlaylistCodec {
            ...
            .forEach { line ->
                when {
                    ...
                    line.startsWith("#") -> Unit
                        ...
                        entries += if (info == null) {
                            M3uPlaylistEntry(title = line.substringAfterLast('/'), artist = "Unknown Artist", durationSeconds = -1L, uri = line)
                        } else {
                            info.copy(uri = line)
                        }
                        pendingInfo = null
    ...
    fun export(playlist: M3uPlaylist): String = buildString {
        appendLine("#EXTM3U")
        playlist.entries.forEach { entry ->
            appendLine("#EXTINF:${entry.durationSeconds},${entry.artist} - ${entry.title}")
    ...
    private fun String.parseExtInf(): M3uPlaylistEntry {
        ...
        return M3uPlaylistEntry(title = title, artist = artist, durationSeconds = duration, uri = "")
    }
}