...
Library browsing uses a typed `LibraryQuery` through the repository boundary. Room applies filters, browse values, supported sort orders, and `LIMIT`/`OFFSET` before data reaches the ViewModel; Compose accumulates only requested 100-song windows and asks for another window near the visible list end. Playlist and queue commands resolve their entry URI sets through targeted repository lookups, so playback does not require a complete UI-state library snapshot. Composite Room indexes cover favourite, play-history, artist, album, and duration browse/sort paths. Contains search remains SQL-bound but uses `LIKE` across persisted metadata, so it is not B-tree indexed.
